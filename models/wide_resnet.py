...
class VAE_128(nn.Module):
    def __init__(self):
        ...
        dim_mu = 640
        dim_logvar = 640
        self.fc1 = nn.Linear(4*128*128, dim_mu)
        self.fc2 = nn.Linear(4*128*128, dim_logvar)

        self.fc3 = nn.Linear(dim_mu, 4*128*128)
        self.conv3 = nn.Conv2d(
            in_channels  = 4,
            out_channels = 4,
            kernel_size  = (3,3),
...
class Wide_ResNet(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, data_shape):
        ...
        self.linear = nn.Linear(n_output_bn, self.num_classes)
    ...
    def forward(self, x):
        ...
        # TODO salima 0803
        vec = F.avg_pool2d(out, 8) # to try

        # vec = F.max_pool2d(out, 8) # test acc fit (ok)
...
class Wide_ResNet_preMixup_final(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, data_shape):
        self.num_classes = num_classes
        super(Wide_ResNet_preMixup_final, self).__init__()
        self.in_planes = 16
    ...
    def forward(self, x):
        ...
        hidden_vector = self.conv1(x)
        
        hidden_vector = self.layer1(hidden_vector)
        
        return hidden_vector


class Wide_ResNet_postMixup_final(nn.Module):
    def __init__(self, depth, widen_factor, dropout_rate, num_classes, data_shape):
        self.num_classes = num_classes
        super(Wide_ResNet_postMixup_final, self).__init__()
    ...
    def forward(self, x):
        ...
        # TODO:
        vec = F.avg_pool2d(out, 8) # TODO: salima 要用哪一個?
...
# class old_Wide_ResNet_preMixup(nn.Module):
...
# class old_Wide_ResNet_postMixup(nn.Module):
#    def __init__(self, depth, widen_factor, dropout_rate, num_classes):
